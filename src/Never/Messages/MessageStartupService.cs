...
namespace Never.Messages
{
    ...
    /// event事件启动
    /// </summary>
    internal class MessageStartupService
    {
        public class MessageHandlerCtorProcessor : Never.Startups.ITypeProcessor
        {
            /// <summary>
            /// 事件类型
            /// </summary>
            private readonly static Type messageHandlerType = typeof(IMessageSubscriber);
            ...
            public MessageHandlerCtorProcessor(IEnumerable<Type> ignoreTypes)
            {
            ...
            public void Processing(IApplicationStartup application, Type type)
            {
                ...
                if (!ObjectExtension.IsAssignableToType(messageHandlerType, type))
                    return;
