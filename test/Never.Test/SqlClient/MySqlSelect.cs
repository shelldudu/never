    ...
    public class MySqlSelect
    {
        ...
        public void testSelect1()
        {
            ...
            var more = dao.ToEasyLinqDao(new { Id = 1, IdArray = new[] { 22, 23, 24, 25 }.ToNullableParameter(), Name = "ee" })
               ...
               .ToEnumerable()
               .Where((p, t) => t.Id >= p.Id)
               .AndExists<MySqlUser>("t1").Where((p, t, t1) => t.Id == t1.Id).And((p, t, t1) => t1.Id == 23).ToWhere()
        ...
        {
            var user = new MySqlUser() { Name = "sqlserver", UserId = 22334, AggregateId = NewId.GenerateGuid(), CreateDate = DateTime.Now, EditDate = DateTime.Now };
            var dao = ConstructibleDaoBuilder<MySqlBuilder>.Value.Build();
        ...
        {
            var user1 = new MySqlUser() { Name = "sqlserver", UserId = 22339, AggregateId = NewId.GenerateGuid(), CreateDate = DateTime.Now, EditDate = DateTime.Now };
            var user2 = new MySqlUser() { Name = "sqlserver", UserId = 223310, AggregateId = NewId.GenerateGuid(), CreateDate = DateTime.Now, EditDate = DateTime.Now };
            var dao = ConstructibleDaoBuilder<MySqlBuilder>.Value.Build();
        ...
        {
            var dao = ConstructibleDaoBuilder<MySqlBuilder>.Value.Build();
            var user = dao.ToEasyLinqDao(new { Id = 1 }).Select<MySqlUser>().As("t").ToSingle().Where((p, t) => t.Id == p.Id).GetResult();
        ...
        public void testUpdate2()
        {
            ...
            var sql = dao.ToEasyLinqDao(user)
                .Cached("UUUUUUUU")
                .Update().As("t")
                .InnerJoin<MySqlUser>("t1").On((p, t, t1) => t.Id == t1.Id).And((p, t, t1) => t1.Name != "10123456789").ToUpdate()
                .SetColumn(t => t.Name, p => p.Name)
                .SetColumnWithValue(t => t.Version, user.Version + 1)
                .Where((p, t) => t.Id == p.Id && t.Name == "eee")
                .AndExists<MySqlUser>("t2").Where((p, t, t1) => t.Id == t1.Id).And((p, t, t1) => t1.Id == 23).ToWhere();
        ...
        {
            var dao = ConstructibleDaoBuilder<MySqlBuilder>.Value.Build();
            var user = dao.ToEasyLinqDao(new { Id = 1 }).Select<MySqlUser>().ToSingle().Where((p, t) => t.Id == p.Id).GetResult();
            user.Id = user.Id + 10;
        ...
        public void testDelete2()
        {
            ...
            var sql = dao.ToEasyLinqDao(user).Cached("DDDDDD")
                .Delete().As("t")
                .InnerJoin<MySqlUser>("t1").On((p, t, t1) => t.Id == t1.Id).And((p, t, t1) => t1.Name != "10123456789").ToDelete()
                .Where((p, t) => p.Id == t.Id)
                .AndExists<MySqlUser>("t2").Where((p, t, t1) => t.Id == t1.Id).And((p, t, t1) => t1.Id == 23).ToWhere(); ;
